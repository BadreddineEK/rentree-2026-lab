...
def _load_revenu() -> pd.DataFrame:
    ...
    f = f[(f["GEO_OBJECT"] == "DEP") & (f["FILOSOFI_MEASURE"] == "MED_SL")].copy()
    f["dep"] = f["GEO"].map(_norm_dep)
    f["revenu_median"] = pd.to_numeric(f["OBS_VALUE"], errors="coerce")
    return f[["dep", "revenu_median"]].dropna()
